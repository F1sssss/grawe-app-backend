...
const sql_test = require('../sql_test');
...
describe('Access control queries', () => {
  ...
  beforeAll(() => {
    connection = new DBConnection(sql_test);
  ...
  it('should successfully create a new permission', async () => {
    ...
    const queryResult = await connection.executeQuery('SELECT * FROM gr_permission WHERE route = @route', [
      { name: 'route', type: sql.VarChar, value: '/api/test' },
    ]);
  ...
  it('should create a dummy property for testing purposes', async () => {
    const result = await connection.executeQuery(
      ...
      [
        { name: 'group_id', type: sql.Int, value: groupID },
        { name: 'permission_property_id', type: sql.Int, value: permissionId },
      ],
    );
  ...
  it('should delete user that was made for testing', async () => {
    await connection.executeQuery('DELETE FROM users WHERE id = @id', [{ name: 'id', type: sql.Int, value: user_id }]);
  });

  it('should delete property that was made for testing', async () => {
    await connection.executeQuery('DELETE FROM gr_permission_properties WHERE id = @id', [{ name: 'id', type: sql.Int, value: property_id }]);
  });
