...
const getClientInfo = async (id) => {
  return ({ client, statusCode } = await excecuteClientQueryTemplate('clientInfo.sql', Client(id)));
};

const getClientHistory = async (id, dateFrom, dateTo) => {
  return ({ client, statusCode } = await excecuteClientQueryTemplate('getClientHistory.sql', Client_dateFrom_dateTo(id, dateFrom, dateTo), true));
};
