...
if [ ! -f /var/opt/mssql/data/.initialized ]; then
    ...
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P $SA_PASSWORD -Q "IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = 'GRAWE_DEV') BEGIN CREATE DATABASE GRAWE_DEV; END"
    ...
    for script in $SQL_SCRIPTS; do
        ...
        /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P $SA_PASSWORD -d GRAWE_DEV -i "$script" || {
